...
#include "omp.h"
int main() {
	printf("k^0  pos_peak  neg_peak  pos_theta  neg_theta  separation\n");
	#pragma omp parallel for
	for(int power = -10 ; power<8;power++){
		// Specify simulation parameters
		double theta_i = 20.0;
		double theta_v = -20.0;
		double sigma = 100.0;
		double deltaTheta = 0.02;
		double trans_coeff = 0.5;
		...
		double k_not = 5*pow(2,power);//aqds is around 500
		...
		printf("%g %g %g %g %g %g\n",k_not,peak_pos,peak_neg,theta_pos,theta_neg,peak_separation);
		//printf("\n\n");
		delete [] theta_array;
		delete [] flux_array;
	}
