...
int main() {
	...
	printf("#k  alpha sigma peak_neg peak_pos  theta_neg  theta_pos  peak_separation\n");
	#pragma omp parallel for
	for(int scaling = -8 ; scaling<8;scaling++){
		for(int alpha_scaling = -20; alpha_scaling<20; alpha_scaling+=2){
			for(int sigma_scaling = -10;sigma_scaling<10;sigma_scaling+=2){
				double theta_i = 20.0;
				double theta_v = -20.0;
				double sigma = exp(sigma_scaling);
				...
				double trans_coeff = 0.1*alpha_scaling;
				...
				printf("%g %g %g %g %g %g %g %g\n",k_not,trans_coeff,sigma,peak_neg,peak_pos,theta_neg,theta_pos,peak_separation);
				delete [] theta_array;
				delete [] flux_array;
			}
		}
